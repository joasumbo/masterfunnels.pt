...
import { pedirJson, MODELO_CLASSIFICACAO, type Esquema } from '../lib/ia.js'
...
function construirSistema(taxonomia: { slug: string; nome: string; descricao: string }[]) {
...
function construirEsquema(slugs: string[]): Esquema {
  return {
    type: 'OBJECT',
    properties: {
      classificacoes: {
...
function citacaoELiteral(citacao: string, entrada: Entrada): boolean {
...
async function correr() {
  const taxonomia = await db.select().from(clusters).orderBy(clusters.ordem)
  if (taxonomia.length === 0) {
    throw new Error('Nao ha clusters. Corre primeiro o script da taxonomia.')
  }

  const todas = await db.select().from(respostas)
  const entradas: Entrada[] = todas.map((r) => ({
    id: r.id,
    r1: r.r1Dificuldade,
    r2: r.r2JaTentou,
    r3: r.r3OQueFariaComprar,
  }))

  console.log(`${entradas.length} respostas, ${taxonomia.length} clusters, modelo ${MODELO_CLASSIFICACAO}`)

  const sistema = construirSistema(taxonomia)
  const esquema = construirEsquema(taxonomia.map((c) => c.slug))

  const lotes: Entrada[][] = []
  for (let i = 0; i < entradas.length; i += POR_LOTE) lotes.push(entradas.slice(i, i + POR_LOTE))

  const resultados: Saida['classificacoes'] = []
  let tokensEntrada = 0
  let tokensSaida = 0

  for (let i = 0; i < lotes.length; i += EM_PARALELO) {
    const grupo = lotes.slice(i, i + EM_PARALELO)
    const respostasDoGrupo = await Promise.all(
      grupo.map((lote) => {
        const corpo = lote
          .map(
            (e) =>
    ...
    for (const r of respostasDoGrupo) {
      resultados.push(...r.dados.classificacoes)
  ...
  await db.delete(classificacoes)
  for (let i = 0; i < linhas.length; i += 40) {
    await db.insert(classificacoes).values(linhas.slice(i, i + 40))
  }
  ...
  console.log(`\n${total} classificacoes gravadas`)
  console.log(`   confianca media ${media}`)
